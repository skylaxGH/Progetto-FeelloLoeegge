    ...
    , ui(new Ui::MainWindow)
{
    ...
    this->setFixedSize(this->size());  //La grandezza della finestra non puÃ² essere modificata (dimensione fissa)

    ui->volumeSlider->setVisible(false);  //Lo slider del volume di default non Ã¨ visibile
}
...
void MainWindow::on_btnStop_clicked() {
    ...
    if(player->isAudioAvailable() == false) {  //In caso il file audio non sia disponibile per la riproduzione..
        ...
        player->stop();             //Negli altri casi stoppo la riproduzione, do valore nullptr a setMedia per segnalare che non Ã¨ disponibile
...
void MainWindow::on_btnCreaplaylist_clicked() {
    nRequest->setModal(true);   //Imposto la finestra parente non cliccabile finchÃ¨ la nuova finestra non finisce il suo corso
    nRequest->show();
}
...
void MainWindow::updateName(QString name) {
    ...
    QString mod = "/" + name + ".txt";  //Per costruire la path assoluta per il file di testo aggiungo il nome del file piÃ¹ lo slash ed il .txt
...
void MainWindow::on_actionApri_un_file_triggered() { //Imposto tutte le azioni della Menu Bar collegandole alle funzioni giÃ  create
